...
import { Plus, Search, ShieldCheck, AlertTriangle, ChevronLeft, ChevronRight, Truck } from 'lucide-react'
...
/**
 * Accounting vendor directory — live AP from shared getPartnerBalances (COA 2000).
 */
export default function AccountingVendorsPanel({ language = 'en' }) {
  ...
  const { data, isLoading, isError, error, refetch } = useAccountingQuery({
    queryKey: ['accounting-vendors-directory', search, page, sort, order, hasOpenBalance, overdueOnly, isActive, city],
    queryFn: () => api.get('/accounting/vendors', {
      params: {
  ...
  const rows = data?.rows || data?.vendors || data?.suppliers || []
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-end gap-2">
        <button
          ...
          onClick={() => navigate('/app/dashboard/suppliers/new?returnTo=/app/dashboard/accounting/vendors')}
        >
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        {[
          [isAr ? 'الموردون' : 'Vendors', totals.vendorCount ?? pagination.total],
          [isAr ? 'ذمم دائنة' : 'Payables', totals.payablesSum, true],
                ...
                <thead>
                  <tr>
                    <th className={salesThClass}>
                      <button type="button" onClick={() => toggleSort('nameEn')}>{isAr ? 'المورد' : 'Vendor'}{sortMark('nameEn')}</button>
                    </th>
                    <th className={salesThClass}>{isAr ? 'الرقم الضريبي' : 'VAT No'}</th>
                    <th className={salesThClass}>{isAr ? 'الهاتف' : 'Phone'}</th>
                    <th className={salesThClass}>
                      <button type="button" onClick={() => toggleSort('outstanding')}>{isAr ? 'الدائن' : 'Payable'}{sortMark('outstanding')}</button>
                    </th>
                    <th className={salesThClass}>
          ...
          {pagination.pages > 1 ? (
            <div className="flex items-center justify-between gap-3 border-t border-slate-100 px-4 py-3 text-sm dark:border-white/10">
              <p className="text-slate-500">
                {isAr
                  ? `${pagination.total} مورد · صفحة ${pagination.page} من ${pagination.pages}`
                  : `${pagination.total} vendors · page ${pagination.page} of ${pagination.pages}`}
              </p>
