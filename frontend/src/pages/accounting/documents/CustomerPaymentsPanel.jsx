...
export default function CustomerPaymentsPanel({ language = 'en' }) {
  ...
  const { data: vouchers = [], isLoading } = useQuery({
    ...
    queryFn: () => api.get('/vouchers', {
      params: { type: 'receive', partyType: 'customer', limit: 100 },
    }).then((r) => r.data),
  })
  ...
  const handleCreate = () => {
    ...
    if (!form.partyName.trim() || !Number.isFinite(amount) || amount <= 0) {
      toast.error(isAr ? 'أكمل العميل والمبلغ' : 'Complete customer and amount')
      return
    }
  ...
  if (showForm) {
    return (
      <AccountingDocumentShell
        language={language}
        eyebrow={isAr ? 'مدفوعات العملاء' : 'Customer payments'}
        title={isAr ? 'دفعة جديدة' : 'New payment'}
        subtitle={isAr ? 'استلام / Receive' : 'Receive payment'}
      ...
      >
        {formTab === 'details' ? (
          ...
          <div className="rounded-2xl border border-dashed border-slate-200 bg-slate-50/60 p-8 text-center text-sm text-slate-500 dark:border-white/10 dark:bg-white/[0.02]">
            {isAr
              ? 'يُنشأ قيد اليومية تلقائياً عند التأكيد (مدين: إيصالات معلقة — دائن: ذمم مدينة).'
              : 'Journal entry is generated on confirm (Debit: Outstanding Receipts — Credit: Accounts Receivable).'}
          </div>
        )}
      </AccountingDocumentShell>
    )
    ...
    <div className="space-y-4">
      ...
      <div className={listShellClass}>
        {isLoading ? (
          ...
          <ResponsiveDataList
            items={filtered}
            empty={<p className={emptyStateClass}>{isAr ? 'لا توجد مدفوعات' : 'No customer payments yet'}</p>}
            renderCard={(row) => (
                  ...
                  <tr key={row._id} className={salesTrClass}>
                    ...
                    <td className={salesTdClass}>
                      <span className={softChipClass}>
                        {resolvePaymentRibbonStep(row) === 'draft'
                          ? (isAr ? 'مسودة' : 'Draft')
                          : (isAr ? 'مرحّلة' : 'Posted')}
                      </span>
                    </td>
