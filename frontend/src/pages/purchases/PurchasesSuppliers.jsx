    ...
    <div className="space-y-6">
      ...
      {showAddSupplierModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4 backdrop-blur-sm">
          <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} className={`${shell} w-full max-w-md p-6 space-y-4`}>
            <div className="flex items-center justify-between border-b border-slate-100 pb-3 dark:border-white/[0.08]">
              <div>
                <p className="text-[10px] font-medium uppercase tracking-[0.2em] text-slate-400">{language === 'ar' ? 'الموردون' : 'Suppliers'}</p>
                <h3 className="text-base font-bold text-slate-950 dark:text-white">{language === 'ar' ? 'إضافة مورد جديد' : 'New Supplier'}</h3>
              </div>
              <button type="button" onClick={() => setShowAddSupplierModal(false)} className="rounded-lg p-1 text-slate-400 hover:bg-slate-100">
                <X className="h-4 w-4" />
              </button>
            </div>
            ...
            <div className="grid grid-cols-1 gap-3 text-xs">
              <div>
                <label className="label">{language === 'ar' ? 'اسم المورد (EN)' : 'Supplier Name (EN)'} *</label>
                <input className="input" value={supplierForm.nameEn} onChange={(e) => setSupplierForm((p) => ({ ...p, nameEn: e.target.value }))} />
              </div>
              <div>
                <label className="label">{language === 'ar' ? 'اسم المورد (AR)' : 'Supplier Name (AR)'}</label>
              ...
              <div>
                ...
                <input className="input" placeholder="3xxxxxxxxxxxxxx" value={supplierForm.vatNumber} onChange={(e) => setSupplierForm((p) => ({ ...p, vatNumber: e.target.value }))} />
              </div>
            </div>

            <div className="flex justify-end gap-2 border-t border-slate-100 pt-3 dark:border-white/[0.08]">
              <button type="button" onClick={() => setShowAddSupplierModal(false)} className={ghostBtn}>
                {t('cancel')}
              </button>
              <button
                type="button"
                ...
                className={primaryBtn}
              >
                {addSupplierMutation.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4 opacity-80" />}
