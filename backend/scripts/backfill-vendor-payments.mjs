#!/usr/bin/env node
/**
 * Backfill outbound AccountPayment rows from orphan VendorBillPayment /
 * VendorAdvance / PurchaseOrderAdvance journals.
 *
 *   node backend/scripts/backfill-vendor-payments.mjs
 *   node backend/scripts/backfill-vendor-payments.mjs golden-touch
 *   node backend/scripts/backfill-vendor-payments.mjs --apply golden-touch
 */
...
async function main() {
  ...
  for (const t of tenants) {
    ...
    const report = await backfillVendorPaymentsFromJournals(t._id, {
      dryRun: !apply,
      limit: 2000,
    });
    console.log(`\n=== ${label} ===`);
